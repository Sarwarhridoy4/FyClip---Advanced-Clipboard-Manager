...
func TestValidateClipboardContentRejectsOversizedPayloads(t *testing.T) {
	m := &Manager{}
	...
	tests := []struct {
		...
		{
			name: "text",
			item: Item{Type: TypeText, Content: strings.Repeat("a", MaxClipboardTextSize+1)},
		},
		{
			name: "html content",
			item: Item{
				Type:        TypeHTML,
				Content:     "small",
				HTMLContent: strings.Repeat("b", MaxClipboardTextSize+1),
			},
		},
		{
			name: "image",
			item: Item{Type: TypeImage, ImageData: strings.Repeat("c", MaxClipboardImageSize+1)},
		},
		{
			name: "file path",
			item: Item{
				Type: TypeFile,
				FileInfo: &FileInfo{
					Path: "/" + strings.Repeat("d", MaxClipboardPathSize),
				},
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := m.validateClipboardContent(&tt.item); err == nil {
				t.Fatalf("expected oversized %s content to be rejected", tt.name)
			}
		})
	}
}

func TestSecureCopyToMonitorUsesActualClipboardHash(t *testing.T) {
	monitor := &Monitor{stopChan: make(chan struct{})}
	m := &Manager{monitor: monitor}

	item := &Item{
		Type:    TypeText,
		Content: "secret clipboard value",
	}

	m.secureCopyToMonitor(item, false)

	expected := sha256.Sum256([]byte(item.Content))

	monitor.mu.RLock()
	defer monitor.mu.RUnlock()

	if !monitor.programmaticCopy {
		t.Fatal("expected programmatic copy flag to be set")
	}
